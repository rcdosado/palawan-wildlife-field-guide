...
class TestSpeciesListView(TestCase):
	def test_list_species(self):
		name = 'django'
		request = RequestFactory().get('/species/')
		view = SpeciesListView(template_name='species/list_species.html')
		view = setup_view(view, request, name=name)

		import pdb; pdb.set_trace()
		self.assertEqual(view.model.__name__, 'Species')
		self.assertEqual(view.request.method, 'GET')
		self.assertEqual(view.template_name, 'species/list_species.html')
		# context = view.get_context_data()
		# self.assertEqual(context['name'], name)

class TestSpeciesDetailView(TestCase):
	pass
